...
import { Add, Download, Delete, Replay, Search, Pending } from '@mui/icons-material'
...
const Brand = () => {
        ...
        },
        {
            ...
            renderCell: (params) => {
                if (params.value !== null) {
                    return <Box sx={{ p: 2 }}><img src={uploadUrl + params.value} alt='Logo' width='40' height='auto' /></Box>
                }
                return <Box sx={{ p: 2 }} > <img src='/noimg.jpeg' alt='Logo' width='40' height='auto' /></ Box>
            }
        },
        {
            field: 'title',
    ...
    const doUpdate = (data) => {
        ...
        apiService.putWithMedia(`brand/${data.id}`, formData)
            .then(res => {
                loadData()
                hideLoading()
                showNotification(
    ...
    const handleKeyDown = (e) => {
        if (e.key === 'Enter') {
            apiService.get(`brand?keyword=${keyword}`)
                .then(res => {
    ...
    const doAddNew = (data) => {
        showLoading()
        const formData = dataToFormData(data)
        apiService.postWithMedia(`brand`, formData)
            .then(res => {
                loadData()
                hideLoading()
                showNotification(
    ...
    const doDelete = () => {
        showLoading()
        apiService.delete(`brand/${selectedRows}`)
            .then(res => {
                loadData()
                setSelectedRows([])
                closeConfirm()
    ...
    const handleExport = () => {
        apiService.exportExcel('export/brand')
            .then(res => {
    ...
    return (
        <Box sx={{
            ...
            <Box sx={{ mb: 2 }}>
                <Typography variant='h5'>Danh mục nhãn hiệu</Typography>
            </Box>
            <Box sx={{ mb: 2 }}>
