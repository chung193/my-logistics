...
const Role = () => {
    ...
    const processRowUpdate = (newRow, oldRow) => {
        ...
        apiService.put(`role/${newRow.id}`, newRow)
            .then(res => {
    ...
    const handleKeyDown = (e) => {
        if (e.key === 'Enter') {
            apiService.get(`role?keyword=${keyword}`)
                .then(res => {
    ...
    const doDelete = () => {
        showLoading()
        apiService.delete(`role/${selectedRows}`)
            .then(res => {
                loadData()
                setSelectedRows([])
                closeConfirm()
    ...
    const handleExport = () => {
        apiService.exportExcel('export/role')
            .then(res => {
    ...
    return (
        <Box sx={{
            ...
            <Box sx={{ mb: 2 }}>
                <Typography variant='h5'>Nhóm quyền</Typography>
            </Box>
            <Box sx={{ mb: 2 }}>
