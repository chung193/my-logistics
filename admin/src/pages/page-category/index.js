...
const PageCategory = () => {
    ...
    const processRowUpdate = (newRow, oldRow) => {
        ...
        apiService.put(`page-category/${newRow.id}`, newRow)
            .then(res => {
    ...
    const handleKeyDown = (e) => {
        if (e.key === 'Enter') {
            apiService.get(`page-category?keyword=${keyword}`)
                .then(res => {
    ...
    const doAddNew = (data) => {
        showLoading()
        apiService.post(`page-category`, data)
            .then(res => {
                loadData()
                hideLoading()
                showNotification(
    ...
    const doDelete = () => {
        showLoading()
        apiService.delete(`page-category/${selectedRows}`)
            .then(res => {
                loadData()
                setSelectedRows([])
                closeConfirm()
    ...
    const handleExport = () => {
        apiService.exportExcel('export/page-category')
            .then(res => {
    ...
    return (
        <Box sx={{
            ...
            <Box sx={{ mb: 2 }}>
                <Typography variant='h5'>Danh mục trang</Typography>
            </Box>
            <Box sx={{ mb: 2 }}>
