...
const Page = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            filterable: false,
            renderCell: (params) => (
                <Chip label={params.row?.category?.title} color='success' />
            )
        },
        {
            field: 'slug',
    ...
    const doUpdate = (data) => {
        ...
        apiService.putWithMedia(`page/${data.id}`, formData)
            .then(res => {
                loadData()
                hideLoading()
                showNotification(
    ...
    const processRowUpdate = (newRow, oldRow) => {
        ...
        apiService.put(`page/${newRow.id}`, newRow)
            .then(res => {
    ...
    const handleKeyDown = (e) => {
        if (e.key === 'Enter') {
            apiService.get(`page?keyword=${keyword}`)
                .then(res => {
    ...
    const doAddNew = (data) => {
        showLoading()
        const formData = dataToFormData(data)
        apiService.postWithMedia(`page`, formData)
            .then(res => {
                loadData()
                hideLoading()
                showNotification(
    ...
    const doDelete = () => {
        showLoading()
        apiService.delete(`page/${selectedRows}`)
            .then(res => {
                loadData()
                setSelectedRows([])
                closeConfirm()
    ...
    const handleExport = () => {
        apiService.exportExcel('export/page')
            .then(res => {
    ...
    return (
        <Box sx={{ width: '100%' }}>
            <Box>
                <Typography variant='h5' sx={{ mb: 2 }}>Danh mục trang</Typography>
            </Box>
            <Box>
