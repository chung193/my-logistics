...
const PostCategory = () => {
    ...
    const processRowUpdate = (newRow, oldRow) => {
        ...
        apiService.put(`post-category/${newRow.id}`, newRow)
            .then(res => {
    ...
    const handleKeyDown = (e) => {
        if (e.key === 'Enter') {
            apiService.get(`post-category?keyword=${keyword}`)
                .then(res => {
    ...
    const doAddNew = (data) => {
        showLoading()
        apiService.post(`post-category`, data)
            .then(res => {
                loadData()
                hideLoading()
                showNotification(
    ...
    const doDelete = () => {
        showLoading()
        apiService.delete(`post-category/${selectedRows}`)
            .then(res => {
                loadData()
                setSelectedRows([])
                closeConfirm()
    ...
    const handleExport = () => {
        apiService.exportExcel('export/post-category')
            .then(res => {
    ...
    return (
        <Box sx={{
            ...
            <Box sx={{ mb: 2 }}>
                <Typography variant='h5'>Danh mục bài viết</Typography>
            </Box>
            <Box sx={{ mb: 2 }}>
