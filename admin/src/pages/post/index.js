...
const Post = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            editable: false,
            renderCell: (params) => (
                <Chip label={params.row?.category?.title} color='secondary' />
            )
        },
        {
            field: 'slug',
    ...
    const doUpdate = (data) => {
        ...
        apiService.putWithMedia(`post/${data.id}`, formData)
            .then(res => {
                loadData()
                hideLoading()
                showNotification(
    ...
    const processRowUpdate = (newRow, oldRow) => {
        ...
        apiService.put(`post/${newRow.id}`, newRow)
            .then(res => {
    ...
    const handleKeyDown = (e) => {
        if (e.key === 'Enter') {
            apiService.get(`post?keyword=${keyword}`)
                .then(res => {
    ...
    const doAddNew = (data) => {
        ...
        console.log("Is File:", data.image instanceof File); // Phải là true!

        const formData = dataToFormData(data)
        console.log('====', formData)
        apiService.postWithMedia(`post`, formData)
            .then(res => {
                loadData()
                hideLoading()
                showNotification(
    ...
    const doDelete = () => {
        showLoading()
        apiService.delete(`post/${selectedRows}`)
            .then(res => {
                loadData()
                setSelectedRows([])
                closeConfirm()
    ...
    const handleExport = () => {
        apiService.exportExcel('export/post')
            .then(res => {
    ...
    return (
        <Box sx={{ width: '100%' }}>
            <Box>
                <Typography variant='h5' sx={{ mb: 2 }}>Quản lý bài viết</Typography>
            </Box>
            <Box>
