...
const ProductCategory = () => {
    ...
    const processRowUpdate = (newRow, oldRow) => {
        ...
        apiService.put(`attribute/${newRow.id}`, newRow)
            .then(res => {
    ...
    const handleKeyDown = (e) => {
        if (e.key === 'Enter') {
            apiService.get(`attribute?keyword=${keyword}`)
                .then(res => {
    ...
    const doAddNew = (data) => {
        showLoading()
        apiService.post(`attribute`, data)
            .then(res => {
                loadData()
                hideLoading()
                showNotification(
    ...
    const doDelete = () => {
        showLoading()
        apiService.delete(`attribute/${selectedRows}`)
            .then(res => {
                loadData()
                setSelectedRows([])
                closeConfirm()
    ...
    const handleExport = () => {
        apiService.exportExcel('export/attribute')
            .then(res => {
    ...
    const loadDataFilter = (id) => {
        showLoading()
        apiService.get(`attribute?product_category_id=${id}`)
            .then(res => {
                setData(res.data.data)
                showNotification('Load dữ liệu thành công', 'success')
                hideLoading()
            })
            .catch(err => {
                console.log(err)
    ...
    return (
        <Box sx={{
            ...
            <Box sx={{ mb: 2 }}>
                <Typography variant='h5'>Các trường thông tin</Typography>
            </Box>
            <Box sx={{ mb: 2 }}>
                <Grid container spacing={2}>
                    <Grid size={{ xs: 12, md: 6 }}>
                        <Stack direction={'row'} spacing={2}>
                            ...
                            {category.length > 0 && <FormControl sx={{ minWidth: 300 }}>
